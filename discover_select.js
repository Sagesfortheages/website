...
const hebrewMonths = {
    'בתשרי': 'Tishrei', 'תשרי': 'Tishrei',
    'בחשוון': 'Cheshvan', 'חשוון': 'Cheshvan',
    'בכסלו': 'Kislev', 'כסלו': 'Kislev',
    'בטבת': 'Teves', 'טבת': 'Teves',
    'בשבט': 'Shevat', 'שבט': 'Shevat',
    'באדר א': 'Adar I', 'אדר א': 'Adar I',
    'באדר': 'Adar', 'אדר': 'Adar',
    'באדר ב': 'Adar II', 'אדר ב': 'Adar II',
    'בניסן': 'Nissan', 'ניסן': 'Nissan',
    'באייר': 'Iyar', 'אייר': 'Iyar',
    'בסיוון': 'Sivan', 'סיוון': 'Sivan',
    'בתמוז': 'Tammuz', 'תמוז': 'Tammuz',
    'באב': 'Av', 'אב': 'Av',
    'באלול': 'Elul', 'אלול': 'Elul'
};
...
const hebrewMonthMatch = Object.keys(hebrewMonths).find(month => hebrewDateStr.includes(month));
...
async function loadMarkers() {
  try {
    ...
    allPeople = [...new Map(markers.map(item => [item['person'], item])).values()];
    allPeople.sort((a, b) => a.person.localeCompare(b.person));
    uniqueNames = allPeople.map(p => p.person.toLowerCase());

    populateSuggestions();

    document.getElementById('month').value = getMonthValueByName(englishDate.split(' ')[1].trim());
    updateDays();
    ...
    if(page.isFirstVisit) {
        showCustomAlert(`Search for a sage you wish to learn more about. <br><br>You can see what the hebrew date is and which sages have a birthday or yahrzeit today.<br><br>You can change the date using the Date Selection buttons, to see birthdays and yahrtzeits on other days.<br><br>Click on a person to see their biography page.`, '2vmin')
    }
}
...
function updateBirthdayList(englishDate){
    document.getElementById('birthdays-list-heading').innerText = `Birthdays For ${englishDate}`;
    ...
    const birthdayBoys = markers.filter(marker => {
        ...
        return multipleDates.some(datePart => {
            ...
            const dayMonth = dayMonthMatch ? dayMonthMatch[0].toLowerCase() : trimmedDatePart;
            return dayMonth === searchDate;
        });
    });
    let uniquebirthdayBoys = [...new Map(birthdayBoys.map(item => [item['person'], item])).values()];
...
function updateYahrtzeitList(englishDate){
    document.getElementById('yahrtzeits-list-heading').innerHTML = `Yahrtzeits For ${englishDate}`;
    ...
    let uniqueyahrtzeitBoys = [...new Map(yahrtzeitBoys.map(item => [item['person'], item])).values()];
